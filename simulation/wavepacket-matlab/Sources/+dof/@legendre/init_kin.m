...
if output
    prt.disp ('***************************************************************')
    prt.disp ( ['Kinetic energy for Gauss-Legendre dof: ' obj.label] )
    prt.disp ('***************************************************************')
    prt.disp ('                                                  ')
    prt.disp ('        1   -2   ^ 2                              ')
    prt.disp (' T  =  --- R     L                                ')
    ...
    if isempty(obj.R_dof)
        prt.disp ( ['where R is chosen as constant: ' num2str(obj.R_0)] )
    else
        prt.disp ( ['where R is taken from the DOF: ' num2str(obj.R_dof)] )
    end
