...
prt.disp ('Square Root Balanced Truncation (SRBT)')
prt.disp ('======================================')

% Cholesky factorization of W_C
prt.disp ('   ')
prt.disp ('Trying Cholesky factorization of Gramian W_C = X*X^T')
X = cholesky (WC);

% Cholesky factorization of W_O
prt.disp ('   ')
prt.disp ('Trying Cholesky factorization of Gramian W_O = Y*Y^T')
Y = cholesky (WO);
...
prt.disp ('   ')
prt.disp ('Singular value decomposition Y^T*X = U*Sigma*V^T')
M = Y'*X;
[U,Sigma,V] = svd(M);
Sigma = diag(Sigma);
threshold = max(size(M))*eps(norm(M)); % from Matlab's "rank" function
r = nnz(Sigma>=threshold);
prt.disp (['Largest singular value                    : ' num2str(max(Sigma))])
prt.disp (['Smallest singular value                   : ' num2str(min(Sigma))])
prt.disp (['Value of threshold used                   : ' num2str(threshold)])
prt.disp (['Dimension of the matrices X and Y         : ' int2str(size(X,1))])
prt.disp (['Number of singular values above threshold : ' int2str(r)])
prt.disp ( 'Neglecting all singular values below threshold!')
prt.disp ('   ')
...
try % for positive definite W
    X=chol(W,'lower');
    prt.disp ('Cholesky factorization of Gramian appears to work fine')
catch % for non-positive definite W
    [eigvec,eigval]=eig(W);
    eigval = diag(eigval);
    prt.disp ( 'Gramian is not positive definite' )
    prt.disp (['Largest eigenvalue                    : ' num2str(max(eigval))])
    prt.disp (['Smallest eigenvalue                   : ' num2str(min(eigval))])
    prt.disp (['Value of threshold used               : ' num2str(threshold)])
    prt.disp (['Number of eigenvalues below threshold : ' int2str(nnz(eigval<threshold))])
    prt.disp ( 'Replacing all eigenvalues below threshold!')
