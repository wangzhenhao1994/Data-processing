...
windowFre = op.new_sheet('w',lname='H2_ground')
...
class Morse:
    ...
    def plot_V(self, ax, F, **kwargs):
        """Plot the Morse potential on Axes ax."""

        ax.plot(self.r/5.29177210903E-11, self.V / FAC/8065.73 + self.Te-15.42593-1.602176634E-19*5.143E11*F*(self.r-self.r[0])*6.24181E+18, **kwargs)
...
# Atom masses and equilibrium bond length for (1H)(Hl).
mA, mB = 1., 1.
X_re = 0.74144E-10
X_Te = 0
...
#X2 = 
X.make_rgrid(rmin=0.205E-10)
X.V = X.Vmorse(X.r)
fig, ax = plt.subplots()
X.plot_V(ax, F=0, color='k')
X.plot_V(ax, F=0.18, color='r')
X.plot_V(ax, F=0.08, color='b')
#X.draw_Elines(range(15), ax)
