...
class FFT_ionS():
    ...
    def FFT(self): #https://www.researchgate.net/post/What-are-the-basic-differences-between-FFT-and-DFT-and-DCT# do FFT for total spectra
        for gas in self.specBigBottleB.keys():
            ...
            #y,t = self.inter_padding(self.specBigBottleB[gas],self.rebin_delay,paddingSize=0)
    ...
    def inter_window(self, data, delay, windowSize=0, direction='left', useWindow = True, phaseCompensate = True, smooth = True):
        ...
        self.windowSize = windowSize
        if smooth:
            data=sps.savgol_filter(data, window_length=3, polyorder=1,deriv=1,delta=1, mode='interp')
        if direction == 'left':
    ...
    def inter_window2(self, data, delay, windowSize=0, direction='left', useWindow = True):
        '''
        windowSize is in fs. Replace the window with the amplitude after the window.
        '''
        __len = np.size(data)
        windowSize = int(windowSize*1e-15/self.stepSize)
        refPos = np.arange(__len)
        if direction == 'left':
    ...
    def show_FFT(self):
        ...
        for gas in self.fftSB.keys():
            ...
            P_window =  np.array([np.mean(np.where(P_inter<0,P_inter+np.pi,P_inter),axis=0), np.std(np.angle(self.fftSB[_preP+gas+'_fft']),axis=0)])
    ...
    def show_Spectra(self, shift=0):
        ...
        for gas in self.specBigBottleB.keys():
            ...
            if gas == 'Ch0':
                gas='Ch2'
                ax.plot(delay*10**15+shift,
                     self.specBigBottleB[gas]/(np.amax(self.specBigBottleB[gas][5500:6000])-np.amin(self.specBigBottleB[gas][5500:6000])), 'black', label=gas)
                gas='Ch6'
                ax.plot(delay*10**15+shift,
                     self.specBigBottleB[gas]/(np.amax(self.specBigBottleB[gas][5500:6000])-np.amin(self.specBigBottleB[gas][5500:6000])), label=gas)
                gas='Ch8'
                ax.plot(delay*10**15+shift,
                     self.specBigBottleB[gas]/(np.amax(self.specBigBottleB[gas][5500:6000])-np.amin(self.specBigBottleB[gas][5500:6000])), label=gas)
            #ax.plot(delay*10**15+shift,
    ...
    def fit_phase(self, x, y, ww0):
        ...
        #        phi = params[i+1]
        #        w0 = params[i+2]#ww0[0]
        #        y = amp*np.sinc(np.pi*(x-w0)/self.dw)*np.exp(1j*(np.pi*(x-w0)/self.dw)+phi)
        #    return y-data
        ...
        def fitFunc(params, x, data):
            ...
            n = params['n'].value
            n2 = params['n2'].value
            model = amp*np.sinc(np.pi*(x-w0)/self.dw*n)*np.exp(1j*(np.pi*(x-w0)/self.dw*n2+phi))
        ...
        params = Parameters()
        params.add('amp', value= 1, min=0, max=1E5) #value is the initial condition
        params.add('phi', value= np.pi, min=0, max=np.pi*2)
        params.add('w0', value= 3655.5, min=3646, max=3665)
        params.add('n', value= 100, min=0)
        params.add('n2', value= 100, min=0) 
...
if __name__ == '__main__':
    ...
    #for ff in [r'pu7.4E+13pr1.2E+15_CO2']:
        ...
        #d.window(windowSize=50, useWindow=False)
        #d.smooth(399)
        #d.rmvExp()
