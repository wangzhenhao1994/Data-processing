...
class FFT_ionS():
    ...
    def read_split(self, overNight = False, firstTry = False, sumNo = 20, usefulRange = [0, 5], cu = False):
        ChN=9 #number of channels measured
        for i in [0,2,4,6,8]:
            x.spectraBottle['Ch'+str(i/2)] = 0
        for fp in self.filepath:
            with h5py.File(fp, 'r+') as f:
                ...
                print(data.shape)
                for i in [0,2,4,6,8]:
                        self.interSpectraBottle['Ch'+str(i/2)] = np.sum(data[i::ChN],0)
                        #iii=data[i::ChN]
                        #for i in range(1,5):
                        #    plt.plot(iii[i*3])
                        #plt.show()
                        self.spectraBottle['Ch'+str(i/2)] = np.sum(data[i::ChN][1:],0)
                #self.findZeroDelay()
                #for i in range(ChN):
        ...
        #self.delay = np.arange(self.spectraBottle['Ch'+str(0)].size)*delayStep
        
        add = lambda i,j, m: x.spectraBottle['Ch'+str(i)]+x.spectraBottle['Ch'+str(j)]*m
        sub = lambda i,j: x.spectraBottle['Ch'+str(i)]-x.spectraBottle['Ch'+str(j)]
        
        #x.spectraBottle['Ch0+1'] = add(0,1,1)#'Mass1+Mass2','Mass1+Mass16', 'Mass1+Mass17', 'Mass16+Mass17','Mass17+Mass18',
    ...
    def useFilter(self, lowcut, highcut, comFre):
        for gas in list(self.spectraBottle.keys()):
            ...
            self.spectraBottle['comfil_'+gas] = self.butter_bandpass_filter(self.spectraBottle[gas], (comFre-60)/33.35641*1e12, (comFre+60)/33.35641*1e12, fs)
    ...
    def findZeroDelay(self):
        zeroDelayIndex = np.array([])
        for gas in list(self.interSpectraBottle.keys()):
            if gas == 'Ch4':
                continue
            fs = 1/(self.delay[1]-self.delay[0])
            filterSpec = self.butter_bandpass_filter(self.interSpectraBottle[gas], 200/33.35641*1e12, 6000/33.35641*1e12, fs)
            zeroDelayIndex=np.append(zeroDelayIndex, np.abs(filterSpec-np.amax(filterSpec)).argmax())
        zeroIndex = int(np.mean(zeroDelayIndex))
        for gas in list(self.interSpectraBottle.keys()):
    ...
    def show_FFT(self):
        ...
        lab=['Mass 1','Mass 2','Mass 18','Mass 17','Mass 16','Mass1+Mass18','Mass1+Mass16', 'Mass1+Mass17', 'Mass16+Mass17','Mass17+Mass18']
        #threshold=[0,0,0,0.1,0]#one hour measurement
        #height=[1,0.5,0.5,3,30]
        threshold=[0,0.005,0.01,0,0.1]
        height=[6,0.8,0.5,1.2,15]
        distance=[5,5,5,5,30]
        i=0
        for gas in self.fftS.keys():
            ...
            #peaks, _ = sps.find_peaks(Y, height=height[i], threshold=threshold[i], width=3)
            ...
            ax.plot(f,np.abs(Y_window), label='window'+label)#gas)#, label=self.trueScanTime)
    ...
    def show_Spectra(self, shift=0):
        ...
        for gas in self.spectraBottle.keys():
            ...
            ax.plot(delay*10**15+shift,
                     self.spectraBottle['window_filter_'+gas], label='window_filter_'+gas)
            #ax.plot(delay*10**15+shift,
    ...
    def showRelativePhase(self, shift=0):
        ...
        bax = brokenaxes(xlims=((50, 100), (300, 350)),ylims=((-1,0.5),))
        ...
        for gas in self.spectraBottle.keys():
            ...
            if 'com' in gas:
                ...
                label = 'filter3550-3750cm_'+label
            ...
            #else:
            #    delay = self.rebin_delay
            if 'Ch3' in gas or 'Ch4' in gas:
                bax.plot(delay*10**15+shift,
                        self.spectraBottle[gas]/10,label=label)
            elif 'Ch2' in gas:
                bax.plot(delay*10**15+shift,
                        self.spectraBottle[gas]*10,label=label)
            else:
        ...
        #plt.ylim([-1.5,1.5])
        #fig.tight_layout()
        plt.savefig("comPhase1816-3647",dpi=720, bbox_inches='tight', pad_inches=0.2)
...
if __name__ == '__main__':
    ...
    f0 = FFT_ionS(filename=filename0, scanTime=20, sampRate=300, dcRange=2, massRange=[5,50])
    f1 = FFT_ionS(filename=filename1, scanTime=20, sampRate=300, dcRange=2, massRange=[5,50])
    ...
    for x in ff:
        ...
        #x.findZeroDelay()
        x.useFilter(10/33.35641*1e12, 6000/33.35641*1e12, comFre=4147)
        #x.useFilter(3581/33.35641*1e12, 3711/33.35641*1e12, comFre=4147)
