...
class FFT_ionS():
    ...
    def __init__(self, filename, scanTime, sampRate, massRange=[5, 50], dcRange=2, cal_mass=[17,18], cal_pixel=[275,469], ChN=10):
    ...
    #def __init__(self, filename, massRange=[5, 50], dcRange=2, cal_mass=[17,18], cal_pixel=[197,279]):
        ...
        self.delay = np.arange(self.scanLength)/self.scanLength*self.scanTime*20*2*3.33564*10**-15
    ...
    def read_split_slidingWindow(self, numRange=45, numEff=37, shift=0):
        ...
        numOverLay=int((self.delay[-1]-1000.69*1e-15)/2/delayStep)+shift
        print(numOverLay)
        for i in range(ChN):
            x.spectraBottle['Ch'+str(i)] = 0
        for fp in self.filepath:
    ...
    def show_FFT(self):
        ...
        for gas in self.fftS.keys():
            ...
            Y=np.where(np.logical_and(f>=10, f<=20000),Y,0)/plotRatio
            Y_filter=np.where(np.logical_and(f>=10, f<=20000),Y_filter,0)/plotRatio
    ...
    def show_Spectra(self, shift=0):
        ...
        lab=['Mass 1','Mass 2','Mass 16/32','Mass 17','Mass 18','Mass2+Mass18','Mass1+Mass16', 'Mass1+Mass17', 'Mass16+Mass17','Mass17+Mass18']
        i=0
        for gas in self.spectraBottle.keys():
            print(gas)
            ...
            #plt.xlim([300,400])
            ax.legend()
            np.savetxt(pl.PureWindowsPath(x.exportPath, 'delayMonitor.dat'), x.spectraBottle[gas])
...
if __name__ == '__main__':
    ...
    filename40= [r'scan_tof_2022-03-27-21-08-14']# 180-165 MCP3300 8e-7 300ps
    
    filename41= [r'scan_tof_2022-03-27-06-25-44']# 180-165 MCP3300 8e-7 300ps
    
    
    
    

    f40 = FFT_ionS(filename=filename40, scanTime=5006, sampRate=300, dcRange=2, massRange=[5,50], ChN=1)
    f41 = FFT_ionS(filename=filename41, scanTime=7506, sampRate=300, dcRange=2, massRange=[5,50])
    ...
    for x in ff:
        ...
        #x.findZeroDelay()
        x.useFilter(1/33.35641*1e12,12000/33.35641*1e12, comFre=[4100,4200])
        ...
        x.window(windowSize=0, direction='left')
        x.show_Spectra()
        plt.show()
