...
class FFT_ionS():
    ...
    def __init__(self, filename, massRange=[5, 50], dcRange=2, cal_mass=[17,18], cal_pixel=[433,484]):
...
if __name__ == '__main__':
    ...
    f1 = FFT_ionS(filename=dataFile, dcRange=2, massRange=[5,50])
    ...
    dataPath = pl.PurePath(dataPath)
    dataFile =  os.path.join(dataPath, yearAndMonth, date, filename+r'.hdf5')
    f2 = FFT_ionS(filename=dataFile, dcRange=2, massRange=[5,50])
    ...
    f2.window(windowSize=141.6, direction='left')
    i=0
    j=0
    step=1
    for void in np.arange(0,1,step):
        i=i+step
        f2.window(windowSize=step, direction='left')
        ff= np.concatenate((f1.spectraBottle["H2O+"],f2.spectraBottle["H2O+"]))
        #ff=f2.padding()
        delayd = np.arange(ff.shape[0])/f1.spectraBottle["H2O+"].shape[0]*(116.7/120*99)*2*3.33564*10**-15
        [t,fre]=f2.FFT(delayd,ff)
        plt.plot(delayd*10**15+132,ff)
        plt.xlabel("Delay/fs")
        plt.show()
        if j==0:
            plt.plot(t[100:3000]*33.35641,fre[100:3000],label=str(i))
            fmax=np.amax(fre[100:3000])
        elif np.amax(fre[100:3000])>fmax:
            plt.plot(t[100:3000]*33.35641,fre[100:3000],label=str(i))
            fmax=np.amax(fre[100:3000])
        else:
            continue
    ...
    def SSS(t, y, factor, windowsize=0, ratio=10):
        '''
        windowsize is window size in fs
        '''
        t=f2.rebin_factor(t,factor)
        y=f2.rebin_factor(y,factor)
        f, t, y = sps.stft(y, fs=1/(
            t[2]-t[1]), noverlap=windowsize-2, nperseg=windowsize, nfft=windowsize*3)
        #self.stftS[gas] = self.stftS[gas][:,::-1]*10
        vmax=abs(y[int(len(f)/40):int(len(f)/15)]).max()
        vmin=abs(y).min()
        norm = cm.colors.Normalize(vmax=vmax, vmin=vmin)
        levels = np.arange(vmin,vmax,(vmax-vmin)/500)
        plt.figure(figsize=(8.0, 5.0))
        plt.contourf(t*10**15+132, f/10**12*33.35641,
                       np.abs(y), levels=levels, cmap='jet', norm=norm)
        #plt.xlim([100, self.delay[-1]])
        plt.ylim([1000, 4500])
        plt.ylabel('Frequency [cm-1]')
        plt.xlabel('Time [fs]')
        plt.clim(0,abs(y[:int(len(f)/15)]).max())
        plt.tight_layout()
        plt.show()
    
    SSS(t=delayd,y=ff,factor=20,windowsize=int(200/600*7000))
