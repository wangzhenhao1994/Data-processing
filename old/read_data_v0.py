...
class FFT_ionS():
    ...
    def __init__(self, filename, massRange=[5, 50], dcRange=2, cal_mass=[17,18], cal_pixel=[275,469]):
    ...
    #def __init__(self, filename, massRange=[5, 50], dcRange=2, cal_mass=[17,18], cal_pixel=[197,279]):
        ...
        self.scanLength = 72000#72000
        self.peakRange = [-15, 15]  # range of the peak
        #self.delay = np.arange(self.scanLength)/self.scanLength*(120.8/126.4)*100*2*3.33564*10**-15 # this is the value used before Sep. 2021.
    ...
    def STFTS(self, windowsize=0, ratio=10):
        ...
        for gas in self.spectraBottle.keys():
            ...
            plt.title(gas+d)
            plt.tight_layout()
            #plt.savefig("sft_18"+d,dpi=720, bbox_inches='tight', pad_inches=0.2)
            plt.show()
...
if __name__ == '__main__':
    ...
    dataPath = pl.PureWindowsPath(
        ...
        r'C:\Users\user\Desktop\Data_newTOF\2021-08\2021-08-22')#170-170
    ...
    datafile =  os.path.join(dataPath, r'scan_tof_2021-08-22-22-11-20.hdf5')#150-150
    ...
    for x in ff:
        x.read_split(overNight = True, firstTry = False, sumNo = 20, usefulRange = [0,1], cu=True)
        #check the spectrum
        for i in range(0,4):
